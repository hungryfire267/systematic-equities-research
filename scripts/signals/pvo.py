...
class PVO: 
    def __init__(self): 
        self.volume_df = pd.read_parquet(companies_paths_dict["volume"])
    ...
    def calculate_pvo(self, extremes: tuple[float, float], spans: tuple[int, int]) -> pd.DataFrame: 
        ...
        pvo_df_score = pvo_df.clip(lower=pvo_df.quantile(lower_extreme), upper=pvo_df.quantile(upper_extreme)) # Capping the extremes
        pvo_df_rank = cross_sectional_ranking(pvo_df_score, higher_is_better=True)
        return pvo_df_rank
    
    
    def run(self, extreme_list: Sequence[tuple[float, float]], span_list: Sequence[tuple[int, int]]) -> dict: 
        pvo_df_dict = dict()
        for extremes, spans in zip(extreme_list, span_list): 
            keys = (extremes, spans)
